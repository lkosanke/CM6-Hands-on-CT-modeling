---
title: "CM6 Hands on ctsem modelling - Excercise Solution"
...
A one unit increase of Stress leads to a 0.23 unit decrease in Quality.
...
can be understood when considering the Drift Matrix A in the solution to the 
stochastic differential equation: $$\mathrm{e}^{\mathbf{A} \cdot \Delta t_i}$$

As $$a_{11}$$ or $$a_{22}$$ approache 0 the solution approaches 1 (stability):
$$\mathrm{e}^{\mathbf{A} \cdot \Delta t_i} = \mathrm{e}^{0 \cdot \Delta t_i} = 1$$
and as $$a_{11}$$ or $$a_{22}$$ approach negative infinity the solution 
approaches 0 (instability):
$$\mathrm{e}^{\mathbf{A} \cdot \Delta t_i} = \mathrm{e}^{-\infty \cdot \Delta t_i} = 0$$
