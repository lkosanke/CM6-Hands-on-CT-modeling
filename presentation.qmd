...
time points can be interindividually varying
...
converting from the typical long format to wide:
```{.r}
...
```{.r}
df_wide <- ctIntervalise(datawide = wideexample, Tpoints = 4, n
  manifest = 2, manifestNames = c("Y1", "Y2", "Y3"))
```
