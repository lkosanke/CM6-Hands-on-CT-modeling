...
The smaller the absolute values are, the less stable a given construct is.

In this example, the value for LeisureTime -0.02800085 is larger than the one for happiness with -0.08963823. 
This suggests that Leisure time is a more stable construct than happiness.

## With regards to the cross-effects

Cross-effects are also compared with their counterparts in the Drift matrix output. The higher value of drives the causal relationship of two given variables.
...
It appears that LeisureTime drives the relationship, so that more LeisureTime leads to less happiness. However, as the values are really low, there might also be no effect of the variables on another.
...
Interestingly, for the effect of LeisureTime ->  Happiness, the cross-effect value are higher for larger intervals. However, the small values suggest it might be too small in size to be practically relevant.



# Hypothesis testing
Is the cross-effect of LeisureTime -> Happiness different from 0?
```{r}
